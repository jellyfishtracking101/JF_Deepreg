...
class RegistrationModel(tf.keras.Model):
    ...
    def build_loss(self):
        ...
        if "pred_fixed_image" in self._outputs:
            ...
            # fixed_archive = tf.identity(fixed_image)
            self._build_loss(
                name="image",
                inputs_dict=dict(y_true=fixed_image, y_pred=pred_fixed_image),
            )

            # assert_op = tf.keras.backend.print_tensor(tf.reduce_all(fixed_image == fixed_archive))
            # with tf.control_dependencies([assert_op]):
            #     fixed_image = tf.scalar_mul(1.0, fixed_image)
...
class DDFModel(RegistrationModel):
    ...
    def build_model(self):
        ...
        ddf = ddf * tf.concat([tf.ones(ddf[...,:2].shape), tf.zeros(ddf[...,2].shape + (1,))], axis=-1) # BRIAN IMPORTANT MASK Z DDF

        # assert_op = tf.keras.backend.print_tensor(tf.reduce_max(ddf, axis=[1,2,3]))[0]
        # ddf = tf.add(ddf, tf.scalar_mul(0.0, assert_op))
        # ddf = tf.zeros_like(ddf)
